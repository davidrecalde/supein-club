...
**Nueva clase `.img-rounded-80` — solo para las 14 imágenes de `traditions.mdx` (2026-09-21):** David pidió poner "todas las imágenes" al 80% de ancho con esquinas redondeadas; confirmado con él que se refería únicamente a las 14 imágenes recién añadidas en `culture/traditions.mdx`, no un cambio global. En vez de modificar `.img-medium` (usada en muchos otros artículos ya revisados), se creó una clase hermana `.img-rounded-80` (`max-width: 80%`, `border-radius: 12px`, mismo comportamiento responsive) en `global.css`, y se migraron las 14 imágenes de `traditions.mdx` a esa clase. **Extendido a las 3 imágenes restantes (2026-09-21, mismo día)**: David señaló que quedaban 3 imágenes preexistentes sin tocar (セマナ・サンタ, フェリア・デ・アブリル, サン・フェルミン祭) — se les añadió también `class="img-rounded-80"`. Ahora las 17 imágenes de cuerpo del artículo usan la misma clase; las 3 preexistentes siguen sin `<figure>`/caption manual (el script de auto-wrap de `ArticleLayout.astro` les sigue generando el `<figcaption>` a partir del `alt`).
...
**`football/real-sociedad.mdx` — ampliación con datos verificados + 16 imágenes + sección Reala Nippon (2026-09-21):** a petición de David, contrastado contra 3 fuentes (web oficial `realsociedad.eus/es/club/historia`, Wikipedia "Real Sociedad de Fútbol" y "Historia de la Real Sociedad de Fútbol") antes de escribir. **Corrección de un dato erróneo**: el artículo decía que el título de "Real" se concedió el 10 de febrero de 1910 — las 3 fuentes más una búsqueda de verificación independiente coinciden en **11 de febrero de 1910**; corregido en el cuerpo y en la FAQ. **Sección nueva**: el origen del primer título de la historia del club (Copa del Rey 1909), jugado bajo el nombre prestado "Club Ciclista de San Sebastián" por no tener aún el registro legal a tiempo — dato que ya estaba en la FAQ preexistente pero nunca explicado en el cuerpo; ahora tiene su propia subsección con imagen de época. **Sección de Copa del Rey reestructurada**: antes solo cubría 2019-20 y 2025-26 en el cuerpo (1986-87 solo aparecía en la FAQ) — ahora las 3 conquistas "oficiales" (1986-87, 2019-20, 2025-26) tienen su propio párrafo e imagen, dejando 1909 en la sección de fundación por su naturaleza de origen distinta. Añadido dato verificado sobre Luis Arconada (portero icónico de la era 1980-82) y detalle del partido de cada título de liga (fechas, marcadores, alineación). **Nueva sección "日本の公認サポーターズクラブ「Reala Nippon」"**: verificado que es la primera peña oficial de la Real Sociedad en el continente asiático, fundada en 2020 por 7 jóvenes de Tokio/Yokohama, primera presidenta Mio Ortiz (mujer que vivió en Donostia) — fuente: euskalkultura.eus. Su 5º aniversario (2025) coincidió con la gira de la Real por Japón (V-Varen Nagasaki y Yokohama FC, junio 2025) — fuente: búsqueda cruzada de noticiasdegipuzkoa.eus. Enlazada su cuenta de X oficial (@Reala_Nippon, proporcionada por David). **Imágenes**: 16 fotos reales/de archivo (confirmado con David, ninguna generada por IA) — heroImage nuevo (escudo), fundador, foto de época 1909, 3 de la era 80-82, 1 de Xabi Alonso 2002-03, 1 de Griezmann cantera, 3 de las 3 Copas del Rey "oficiales", 2 de Kubo, logo + foto de grupo de Reala Nippon, y el escudo del club en la introducción — captions descriptivos neutros (sin atribución de fuente ni aviso de IA, ya que son fotos de archivo reales sin crédito de fotógrafo conocido). **Corrección adicional tras pregunta de David sobre canibalización con `kubo-takefusa.mdx`**: el párrafo sobre el fichaje de Kubo y la pregunta de FAQ "久保建英はいつレアル・ソシエダに加入しましたか？" eran casi idénticos palabra por palabra en ambos artículos (preexistente, no introducido en este cambio, pero corregido ahora). Acortado el párrafo de `real-sociedad.mdx` a un resumen desde la perspectiva del club con enlace explícito a la guía dedicada, y eliminada la FAQ duplicada de `real-sociedad.mdx` (se mantiene solo en `kubo-takefusa.mdx`, la página dedicada) — mismo criterio que la desduplicación de El Clásico. De paso, corregido el dato del título de 2019-20: la final se disputó en realidad el 3 de abril de **2021** (aplazada por la pandemia), no en 2020 — "34年ぶり" (no "33年ぶり" como se había escrito primero) es la cifra correcta respecto a 1987, y coincide con la FAQ preexistente que ya lo decía bien.

**`football/athletic-club.mdx` — sección "Tokiotarrak" + detalle de fundación (2026-09-21):** a petición de David, verificado antes de escribir (web oficial `athletic-club.eus` bloqueó el fetch directo con 403, así que se verificó vía búsqueda cruzada con múltiples fuentes: aupaathletic.com, Facebook oficial de la peña, YouTube oficial del club). **Enriquecido, no corregido**: la fecha de fundación 5 de abril de 1901 que ya tenía el artículo es correcta (asamblea constituyente en el Café García con 33 socios fundadores) — se añadió el matiz de que el registro legal de estatutos fue posterior, el 28 de agosto de 1901, con la primera asamblea oficial el 5 de septiembre (dos trámites distintos, ambos de 1901, no una contradicción). **Nueva sección**: "Tokiotarrak", la peña oficial de Japón — verificado que fue fundada en 2014 por Masami Murakami tras un viaje de negocios a Bilbao en el que descubrió San Mamés y la cultura vasca; es la peña oficial más alejada de Bilbao del mundo. 2 imágenes reales (confirmado con David, sin aviso de IA). Sin riesgo de canibalización: no existe ningún otro artículo del sitio sobre la afición japonesa del Athletic.

**Nueva clase `.img-80` — imágenes horizontales de `athletic-club.mdx` (2026-09-21):** David pidió poner "todas las imágenes horizontales" de este artículo al 80% para lograr coherencia de tamaños. A diferencia de `.img-rounded-80` (usada solo en `traditions.mdx`, con `border-radius: 12px` propio), aquí se creó `.img-80` (mismo `max-width: 80%`, sin forzar radio de esquina) para no desentonar con el radio por defecto de 8px que ya aplica `.prose figure img` a las imágenes `.img-vertical` que conviven en el mismo artículo. Aplicada a las 11 imágenes horizontales que no tenían clase (ancho completo antes) y a la de Murakami (`tokiotarrak-masami-murakami.webp`), que pasó de `.img-medium` (65%) a `.img-80` para no ser la única discordante. Las 5 imágenes verticales/logo (`img-vertical`) no se tocaron.
...
**Clase `.img-80` extendida a todo el clúster `/football/` (2026-09-21):** David pidió que todas las imágenes horizontales de todos los artículos de football tuvieran el mismo 80% ya aplicado en `athletic-club.mdx`. Auditadas las 22 páginas `.mdx` del clúster con un script (`PIL` para leer dimensiones reales de cada `public/*.webp` referenciado): 146 imágenes en 16 archivos pasaron a `.img-80` — las que no tenían clase (ancho completo), las que estaban en sintaxis Markdown `![alt](src)` (convertidas a `<img>` HTML para poder llevar `class`), y las que ya estaban en `.img-medium`. **No se tocó ninguna imagen ya marcada `.img-vertical`** — esa clase se trata como una decisión editorial de "mantener pequeña" (retratos, escudos, logos), no una descripción literal de su orientación en píxeles; varias de las `.img-vertical` existentes son técnicamente apaisadas (p. ej. escudos) pero se respetó el criterio ya establecido en el trabajo de `athletic-club.mdx`. Tampoco se tocaron imágenes sin clase que resultaron ser verticales (quedan como estaban, a ancho completo — fuera del alcance de "imágenes horizontales").

**`fcbarcelona.mdx` — nueva sección "PENYA F.C.BARCELONA JAPAN" (2026-09-21):** verificado antes de escribir (web oficial de la peña + búsqueda cruzada): única peña de FCB con reconocimiento oficial en Japón, fundada el 13 de marzo de 2003, presidente Hiroki Nakano (中野洋樹), 4 delegaciones (Hokkaido, Shizuoka, Kobe, Barcelona), placa de reconocimiento en el Camp Nou, 20º aniversario en 2023 con camiseta conmemorativa del club. 3 imágenes reales (confirmado con David, sin aviso de IA) — las 2 cuadradas (fundador y reunión) en `.img-vertical`, la panorámica de miembros en `.img-80`. Enlaces a la web oficial y a X (@f_penya). Sin riesgo de canibalización: no existe ningún otro artículo del sitio sobre la afición japonesa del FC Barcelona.

**`real-madrid.mdx` — nueva sección "ペーニャ・マドリディスタ・ニッポナ" (2026-09-21):** la web oficial de Real Madrid (`realmadrid.com/.../penas-real-madrid/japon--nipona`) devolvió error 500 de forma persistente en varios intentos de fetch, así que se verificó vía búsqueda cruzada con múltiples resultados independientes que citaban el mismo contenido oficial. Verificado: única peña oficial del Real Madrid en Japón (y la única compuesta mayoritariamente por socios japoneses), fundada en julio de 1998 a raíz de la 7ª Copa de Europa del club (mayo de 1998); muchos socios se unieron después, en diciembre de 1998, cuando el Real Madrid disputó y ganó la Copa Intercontinental en el Estadio Nacional de Tokio ante el Vasco da Gama. Presidente: Shigeki Kubota (久保田茂樹). 3 imágenes reales (confirmado con David, sin aviso de IA). Enlaces a la página oficial, X (@PMNipona) y Facebook. Sin riesgo de canibalización: no existe ningún otro artículo del sitio sobre la afición japonesa del Real Madrid.

**Bug real en la conversión masiva a `.img-80`: 105 imágenes sin `<figure>`/caption/espaciado (2026-09-21):** David reportó que las imágenes al 80% no tenían ni esquinas redondeadas ni texto descriptivo ni espacio respecto a los párrafos. Causa raíz: el script de auto-wrap de `ArticleLayout.astro` solo envuelve una `<img>` suelta en `<figure>` (generando `<figcaption>` desde el `alt`) si su elemento padre es un `<p>` — eso es cierto para la sintaxis Markdown `![alt](src)` (MDX la compila dentro de un `<p>`), pero al convertir esas imágenes a `<img class="img-80" />` en HTML (para la migración al 80%, ver entrada anterior), MDX las trata como elemento de bloque fuera de cualquier `<p>`, y el script de auto-wrap nunca se dispara: sin `<figure>` no hay ni el `border-radius: 8px` ni el `margin: 32px 0` de `.prose figure`, ni el `<figcaption>`. Las imágenes que ya usaban sintaxis HTML `<img>` desde antes (como en `athletic-club.mdx`, la referencia que dio David) ya estaban envueltas manualmente en `<figure>` en el propio archivo fuente, por eso no mostraban el problema. **Fix**: envueltas explícitamente en `<figure><img/><figcaption>{alt}</figcaption></figure>` las 105 imágenes afectadas en 10 archivos (`el-clasico.mdx`, `fcbarcelona.mdx`, `japan-vs-spain.mdx`, `japanese-players-laliga.mdx`, `la-liga.mdx` —este último con un bug preexistente de una sesión anterior, no de hoy—, `lamine-yamal.mdx`, `real-madrid.mdx`, `spain-national-team.mdx`, `valencia-cf.mdx`, `world-cup-2026.mdx`), incluidas las 2 imágenes que yo mismo había añadido sin envolver en las nuevas secciones de Penya FCB Japan y Peña Madridista Nipona. **Lección**: cualquier `<img>` HTML añadida a un `.mdx` debe envolverse explícitamente en `<figure>...<figcaption>...</figcaption></figure>` en el propio archivo — nunca depender de que el script de auto-wrap la detecte, ya que solo cubre el caso de imágenes en sintaxis Markdown dentro de un `<p>`.

**Imágenes verticales y cuadradas del clúster `/football/` — nueva clase `.img-70` + criterio de clasificación (2026-09-21):** David pidió que todas las imágenes verticales y cuadradas tuvieran también esquinas redondeadas y caption (mismo bug del `<figure>` que las horizontales), que las cuadradas de buena resolución igualaran el ancho de las horizontales (80%) salvo casos como fotos de presidentes de peñas (deliberadamente pequeñas), y que las verticales tuvieran un ancho "proporcionado" respecto a las horizontales. Criterio aplicado (documentado aquí porque implica varias decisiones de diseño no explícitas en la petición):
- **Nueva clase `.img-70`** (`max-width: 70%`, mismo patrón que `.img-80`): para fotos verticales (retrato) que son contenido protagonista — jugadores, momentos históricos, objetos — ni tan pequeñas como `.img-vertical` (45%) ni tan anchas como las horizontales.
- **`.img-80`**: además de horizontales, ahora también para imágenes cuadradas de buena resolución (≥450px) que son "contenido" (fotos de grupo, objetos temáticos) — no retratos de una sola persona en rol administrativo.
- **`.img-vertical` (45%, sin cambios)**: reservada para logos/escudos (detectados por nombre de archivo), fotos de presidentes/fundadores de peñas (a petición expresa de David — "no queremos destacarlos demasiado"), y cualquier imagen que ya estuviera clasificada así con orientación apaisada (se respetó como decisión editorial previa, fuera de alcance de este cambio).
...
**`football/sevilla-fc.mdx` — ampliación con Biris Norte, Antonio Puerta, Unai Emery + 19 imágenes (2026-09-21):** a petición de David, verificado antes de escribir contra `sevillafc.es`, Wikipedia (ES/EN) y una búsqueda cruzada para el artículo de ABC (bloqueado por fetch). **Nueva sección "ビリス・ノルテとビリ・ビリ"**: primera grada "ultras" organizada de España, fundada 1974-75 como "Peña Biri Biri" en honor al extremo gambiano Alhaji Momodo Njie (109 partidos/34 goles, 1973-78) — grupo antirracista/antifascista, ~1.200 miembros. **Nueva sección "悲劇：アントニオ・プエルタの死"**: colapso cardíaco el 25/8/2007 (min. 28, Sevilla-Getafe), fallecimiento el 28/8/2007 a las 14:30, 22 años, dorsal 16 retirado, estatua en la ciudad deportiva (2010), y su fallecimiento motivó que la FIFA exigiera salas de reanimación en sedes de clasificatorios mundialistas. **Unai Emery**: añadida frase explícita situándolo como uno de los entrenadores más importantes de la historia del club (3 Europa League consecutivas 2013-16, récord histórico de 4 títulos del torneo entre Sevilla y Villarreal) — el dato ya estaba en el cuerpo pero sin esa valoración explícita que pidió David. **Nueva subsección "エンブレムの変遙"** en la sección de fundación: primer escudo de Juan Lafita (1905), rediseño de Pablo Rodríguez (16/10/1921, vigente hoy con variaciones), santos patronos, 11 franjas rojiblancas, cambio de color de las iniciales en 1982 (accidental, durante el mosaico del estadio) — verificado que el escudo NO incluye la Giralda ni la Virgen de los Reyes, pese a que la fuente que dio David sugería lo contrario; no se inventó esa asociación. **Corrección descartada**: se verificó la fecha de fundación (25 de enero de 1890, coincide con "Burns Night" escocés) y el palmarés de Copa del Rey (1935/1939/1948/2007/2010) — ambos ya estaban correctos en el artículo, una fuente de verificación cruzada daba datos erróneos (21 de enero; 1953/1969) que se descartaron tras contrastar con múltiples fuentes independientes. 19 imágenes reales (confirmado con David, sin aviso de IA), repartidas en `.img-80`/`.img-70` según orientación.

**Nuevo artículo `football/real-betis.mdx` (2026-09-22):** club que en el hub `/football/` figuraba como "近日公開予定" (sin `href`). Creado tomando `sevilla-fc.mdx` como referencia de estructura (David lo pidió explícitamente), verificado contra `realbetisbalompie.es`, Wikipedia y 3 búsquedas de verificación cruzada antes de escribir. **Datos clave verificados**: fundación como "Sevilla Balompié" (12/9/1907) + "Betis FC" (1909), fusión aprobada 6-8/12/1914, título "Real" concedido el 17/8/1914 (antes de la fusión, no después), origen del término "Balompié" (Mariano de Cavia, 1/8/1908). **Corrección de un dato erróneo encontrado en una fuente de verificación**: la final de Copa del Rey 2004-05 fue contra Osasuna (2-1 tras prórroga, 11/6/2005, Vicente Calderón) — una fuente decía erróneamente "vs Athletic Club"; confirmado con la página dedicada de Wikipedia a esa final antes de escribir. **Interlinking**: `relatedArticles` → `sevilla-fc`, `la-liga`, `spain-national-team`; sección de derbi enlaza a `sevilla-fc.mdx` en vez de duplicar su historia completa (evita canibalización — el derbi ya está desarrollado allí). Actualizado recíprocamente: `sevilla-fc.mdx` ahora enlaza "レアル・ベティス" a `/football/real-betis/` y lo añade a su `relatedArticles`. Actualizada la tabla de 20 clubes de `/football/index.astro` (`href: null` → `/football/real-betis/`). **Peña Beticos Nippon**: datos limitados disponibles (sede Fuchu-shi, Tokio; cuenta oficial de X desde 2018; eventos "Betis Week" en Tokio) — no se encontró fecha de fundación ni presidente verificables, así que la sección se mantuvo deliberadamente breve para no inventar datos. 2 imágenes reales (hero + evolución del escudo) — confirmado con David que son reales, sin aviso de IA.

**`real-betis.mdx` — Alfonso/Finidi/Jarni + sección de Ruiz de Lopera + 14 imágenes (2026-09-22):** a petición de David, verificado antes de escribir (búsquedas cruzadas específicas por jugador, más `en.wikipedia.org/wiki/Manuel_Ruiz_de_Lopera`). **Nueva sección "マヌエル・ルイス・デ・ロペラ"**: rescató al club de la desaparición en 1992 (2 días antes del plazo de conversión a SAD), presidente 1996-2006, impulsó el fichaje récord de Denílson, la Copa del Rey de 2005 y el debut en Champions League 2005-06 — el estadio llevó su nombre 2000-2010. Presentado con matiz: también se menciona el deterioro financiero posterior y su salida en 2010, ya cubierta en la sección de la crisis de 2009 (sin duplicar esa narrativa, solo enlazando ambas). **Trío del 96-97 añadido a 歴代の名選手**: Alfonso Pérez (235 partidos/80 goles en 2 etapas, 2º máximo goleador histórico), Finidi George (ala nigeriano ex-Ajax, fichaje de 1996) y Robert Jarni (lateral croata ex-Juventus, 82 partidos en 2 temporadas) — generación de la final de Copa del Rey de 1997 bajo Lorenzo Serra Ferrer. 14 imágenes reales (confirmado con David, sin aviso de IA) repartidas por todo el artículo.

**Auditoría de internal linking a partir de GSC (2026-09-22):** David compartió un análisis externo de canibalización/oportunidades basado en Search Console. Verificación contra el código real: de 5 "duplicados" señalados, 4 ya estaban resueltos con `_redirects` desde trabajo de consolidación previo (el dato de GSC era ranking residual de la URL vieja, no una página viva compitiendo) — `lessons-tokyo/tokyo-spanish-schools`, `spanish-grammar/verb-conjugation` (2 variantes). `food/spanish-vocabulary` vs `football/spanish-vocabulary` es un falso positivo: son dos artículos reales y distintos (vocabulario de restaurante vs. vocabulario futbolístico), no canibalización. **Hallazgo importante**: dos de las URLs que el análisis marcaba como "reforzar, no crear contenido nuevo" (`/football/la-liga/la-liga-guide/`, `/food/barcelona-restaurants/`) son en realidad redirects 301, no páginas vivas — el refuerzo de enlaces debe apuntar a sus destinos reales (`/football/la-liga/`, `/food/spain-map/barcelona/`). Añadida la única línea de redirect que faltaba: `/living/study/cervantes-webinar-experiencias-espana/` → `/living/cervantes-webinar-experiencias-espana/` (confirmado 404 en vivo antes del fix; solo 1 impresión en GSC, prioridad mínima).

**2 enlaces internos añadidos tras la auditoría de GSC (2026-09-22):** `el-clasico.mdx` → `/football/` (frase añadida al final de la intro, cerrando el enlace unidireccional que ya existía en sentido contrario desde el hub) y `food/souvenirs.mdx` → `/food/spain-map/barcelona/` (frase añadida tras la sección de Enric Rovira, aprovechando que ambas marcas de chocolate mencionadas justo antes — Cacao Sampaka y Enric Rovira — son de Barcelona, en vez de forzar el enlace en un punto sin relación temática). Sin cambios de título/H1/FAQ en ninguno de los dos artículos, así que no hay vector nuevo de canibalización — son enlaces contextuales puntuales, no contenido nuevo compitiendo por keywords.

**"公開日 / 更新日" visible en la cabecera del artículo (2026-09-22):** David auditó el sistema de fechas para señales de "freshness". Diagnóstico: `updatedDate` ya existía en el schema (`content/config.ts`) y `SEO.astro` ya lo usaba correctamente para `dateModified` en el JSON-LD (con fallback a `pubDate` si no está presente) — 8 artículos ya lo usaban. Lo único que faltaba era mostrarlo en la UI: `ArticleLayout.astro` destructuraba `updatedDate` pero nunca lo pintaba. Añadido: la cabecera ahora muestra "更新日：{fecha}" junto a "公開日：{fecha}" cuando `updatedDate` existe y difiere de `pubDate` (si son iguales o `updatedDate` no está, solo se ve "公開日"). **Regla operativa nueva (memoria: `feedback_haz_freshness.md`)**: nunca tocar `pubDate`/`updatedDate` al editar un artículo salvo que David escriba explícitamente "HAZ FRESHNESS" en la misma tarea — evita que la señal de frescura se infle con ediciones menores y pierda credibilidad.

**`villarreal-cf.mdx` — historia de "CA Foghetecaz" añadida a la fundación (2026-09-22):** a petición de David, verificado contra `villarrealcf.es/es/decada-a-decada/`, la ficha oficial de José Calduch y Wikipedia antes de escribir. Ampliada la sección de fundación con datos que ya estaban implícitos pero sin desarrollar: José Calduch Almela (9/3/1900–23/10/1996), fundó el CD Villarreal el 10/3/1923 en la trastienda de la farmacia familiar; el campo (hoy Estadio de la Cerámica) se inauguró el 17/6/1923. **Dato nuevo verificado**: tras desaparecer el CD Villarreal en la Guerra Civil, en 1942 un grupo de 6 jóvenes (Font, Gil, Herrero, Teulet, Catalá, Zaragozá) fundó el "Club Atlético Foghetecaz" — nombre formado con las iniciales de sus apellidos —, rebautizado "CAF Villarreal" en 1950 y finalmente "Villarreal CF" en junio de 1954. Verificado por dos fuentes independientes (web oficial del club + búsqueda cruzada) al detectar que un primer fetch daba una fecha de fundación distinta y claramente errónea (1942 como fundación original) — descartada tras contrastar. No se tocó `pubDate`/`updatedDate` (sin "HAZ FRESHNESS" en la petición).
